...
$allRooms = []; // Default empty array

try {
    $pdo = connect_db();
    if ($pdo) {
        // Get all rooms regardless of occupancy status
        $stmt = $pdo->query("
            SELECT r.room_id, r.number, r.monthly_rental
            FROM rooms r
            ORDER BY r.number
        ");
...
<div id="createResidentModal" class="fixed inset-0 z-50 overflow-auto flex items-center justify-center hidden">
    <div class="modal-content relative bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 md:mx-auto transform transition-all duration-300 ease-in-out opacity-0 translate-y-4 scale-95">
        ...
        <!-- Form -->
        <form id="createResidentForm" method="POST" action="functions/create-functions.php" class="p-6">
            <!-- First & Last Name -->
            ...
            <div class="mb-4">
                ...
                <input type="text" name="student_id" id="student_id" value="<?php echo old('student_id'); ?>" 
                       class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                       required>
                <?php if (hasError('student_id')): ?>
                    <p class="text-red-500 text-xs mt-1 error-message"><?php echo getError('student_id'); ?></p>
                <?php else: ?>
                    <p class="text-red-500 text-xs mt-1 error-message hidden" id="student_id_error"></p>
                <?php endif; ?>
            </div>
            
            <!-- Room Selection - NEW FIELD -->
            <div class="mb-4">
                ...
                <select name="room_id" id="room_id" 
                    ...
                    <?php foreach ($allRooms as $room): ?>
                        <option value="<?php echo $room['room_id']; ?>" <?php echo (old('room_id') == $room['room_id']) ? 'selected' : ''; ?>>
                            Room <?php echo $room['number']; ?> - ₱<?php echo number_format($room['monthly_rental'], 2); ?>/month
                        </option>
                    <?php endforeach; ?>
                ...
                <?php if (hasError('room_id')): ?>
                    <p class="text-red-500 text-xs mt-1 error-message"><?php echo getError('room_id'); ?></p>
                <?php else: ?>
                    <p class="text-red-500 text-xs mt-1 error-message hidden" id="room_id_error"></p>
                <?php endif; ?>
            </div>

            <div class="mb-4">
            <label for="user_role" class="block text-sm font-medium text-gray-700 mb-1">Role Assignment</label>
            <select name="user_role" id="user_role" 
                    class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    required>
                <option value="">Select a role</option>
                <option value="Resident">Resident</option>
                <option value="Assistant">Resident Assistant</option>
                <option value="Treasurer">Treasurer</option>
                <!-- <option value="Manager">Manager</option> -->
            </select>
            <p class="text-red-500 text-xs mt-1 error-message hidden" id="user_role_error">Please select a role.</p>
            </div>
