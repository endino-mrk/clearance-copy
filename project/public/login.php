...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($pdo) {
            try {
                $sql = "SELECT id, email, password, first_name FROM users WHERE email = :email LIMIT 1"; // Use users table
                ...
                if ($user && password_verify($password, $user['password'])) {
                    ...
                    $_SESSION['user_name'] = $user['first_name']; // Or full name if desired
